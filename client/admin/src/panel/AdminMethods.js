...
export const adminMethods = {
  ...
  setActiveTab(tab) {
    ...
    this.activeTab = tab;
    this.showSearch = false;
    const force = this.shouldForceTabReload(tab);
    const shouldBackgroundLoad = Boolean(this.loadedTabs[tab]) && !force;
    requestAnimationFrame(() => {
      this.loadTab(tab, { background: shouldBackgroundLoad, force });
    });
  },

  shouldForceTabReload(tab) {
    return ["dashboard", "orders", "auctions"].includes(String(tab || ""));
  },
  ...
  normalizeOrderPaymentStatus(value) {
    const normalized = this.normalizeStatus(value || "");
  ...
  normalizeOrder(order) {
    ...
    const customerEmail = String(order.customerEmail || customer.email || "").trim();
    const displayCustomer = customerName || customerEmail || "-";
    ...
    const courier =
      order.courier && typeof order.courier === "object"
        ? {
            provider: String(order.courier.provider || "steadfast"),
            trackingCode: order.courier.trackingCode || null,
            consignmentId: order.courier.consignmentId || null,
            statusCode: order.courier.statusCode || null,
            deliveryStatus: order.courier.deliveryStatus || null
          }
        : {
            provider: "steadfast",
            trackingCode: null,
    ...
    return {
      ...
      paymentStatus: this.normalizeOrderPaymentStatus(order.paymentStatus),
      fulfillmentStatus: this.normalizeStatus(order.fulfillmentStatus || "pending") || "pending",
      ...
      shippingAddress: this.normalizeOrderDraftShipping(order.shippingAddress || {}),
      courier
    };
  },
  ...
  normalizeOrderList(orders) {
    return Array.isArray(orders)
      ? orders.map((order) => this.normalizeOrder(order)).filter(Boolean)
      : [];
  },
  ...
  syncOrderDrafts(orders = this.orders) {
    ...
    (orders || []).forEach((order) => {
      ...
      const orderId = String(normalized?.id || "").trim();
      if (!orderId) return;
      nextDrafts[orderId] = {
  ...
  async loadDashboard(force = false) {
    ...
    const orders = this.normalizeOrderList(ordersRes.items);
  ...
  async loadOrders(resetPage = false) {
    ...
    this.orders = this.normalizeOrderList(response.items);
    this.syncOrderSelection();

    const total = Number(response.total || 0);
    const limit = Math.max(1, Number(response.limit || this.orderFilters.limit || 20));
    const totalPages = Math.max(1, Number(response.totalPages || Math.ceil(total / limit) || 1));
    const page = Math.min(totalPages, Math.max(1, Number(response.page || this.orderFilters.page || 1)));
    ...
    this.orderFilters.limit = limit;
    this.syncOrderDrafts();
  },
  ...
  async saveOrder(order) {
    const current = this.normalizeOrder(order);
    const orderId = String(current?.id || "").trim();
    const draft = this.orderDrafts[orderId];
    if (!orderId || !draft) return;

    try {
      const payload = {};
      if (draft.paymentStatus && draft.paymentStatus !== current.paymentStatus) {
        payload.paymentStatus = draft.paymentStatus;
      }
      if (draft.fulfillmentStatus && draft.fulfillmentStatus !== current.fulfillmentStatus) {
  ...
  normalizeOrderDraftShipping(address) {
    ...
    return {
      ...
      country: clean(source.country) || "BD"
    };
  },
  ...
  applyOrderToList(updated) {
    ...
    const orderId = String(normalized?.id || "").trim();
    if (!orderId) return;
    const targetIndex = this.orders.findIndex((item) => String(item?.id || "").trim() === orderId);
    if (targetIndex >= 0) {
      this.orders[targetIndex] = {
        ...this.orders[targetIndex],
        ...normalized
      };
    }
    if (Array.isArray(this.recentOrders) && this.recentOrders.length) {
      this.recentOrders = this.recentOrders.map((item) =>
        String(item?.id || "").trim() === orderId
          ? {
              ...item,
              ...normalized,
              customer: normalized.displayCustomer,
              status: normalized.fulfillmentStatus || "pending"
            }
          : item
      );
    }
  ...
  async sendOrderToCourier(order, force = false) {
    try {
      ...
      if (response?.order) {
        const updated = this.normalizeOrder(response.order);
