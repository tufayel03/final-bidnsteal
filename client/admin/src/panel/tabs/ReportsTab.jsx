        ...
        <div style={{ display: 'grid', gap: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px' }}>
                <div className="admin-card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <p style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted)', margin: 0 }}>GMV</p>
                    <h4 className="mono" style={{ fontSize: '24px', fontWeight: 800, margin: 0, color: 'var(--text)' }}>{admin.currency ? admin.currency(financialSummary.gmv) : ''}</h4>
                </div>
                <div className="admin-card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <p style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted)', margin: 0 }}>Net Revenue</p>
                    <h4 className="mono" style={{ fontSize: '24px', fontWeight: 800, margin: 0, color: 'var(--text)' }}>{admin.currency ? admin.currency(financialSummary.netRevenue) : ''}</h4>
                </div>
                <div className="admin-card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <p style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted)', margin: 0 }}>Fees Collected</p>
                    <h4 className="mono" style={{ fontSize: '24px', fontWeight: 800, margin: 0, color: 'var(--text)' }}>{admin.currency ? admin.currency(financialSummary.feesCollected) : ''}</h4>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px', alignItems: 'start' }}>
                <div className="admin-card no-pad">
                    <div style={{ padding: '20px', borderBottom: '1px solid rgba(45, 51, 67, 0.8)' }}>
                        <h3 style={{ fontSize: '15px', fontWeight: 800, margin: 0, color: 'var(--text)' }}>Wallets</h3>
                    </div>
                            ...
                            <tbody>
                                {wallets.map(wallet => (
                                    <tr key={wallet.id}>
                                        ...
                                        <td className="mono" style={{ fontSize: '12px', color: 'var(--muted)' }}>{wallet.userId}</td>
                                ...
                                {wallets.length === 0 && (
                                    <tr>
                                        <td colSpan="4" style={{ padding: '40px', textAlign: 'center', color: 'var(--muted)' }}>No wallets found.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                ...
                <div className="admin-card">
                    <h3 style={{ fontSize: '15px', fontWeight: 800, margin: '0 0 16px 0', color: 'var(--text)' }}>Reservations Snapshot</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', textAlign: 'center' }}>
                        <div style={{ padding: '16px', background: 'var(--panel-bg)', borderRadius: '12px', border: '1px solid rgba(45, 51, 67, 0.5)' }}>
                            <p style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--muted)', margin: 0 }}>Active</p>
                            <p className="mono" style={{ fontSize: '20px', margin: '4px 0 0 0', color: 'var(--text)' }}>{admin.number ? admin.number((reservations.active || []).length) : 0}</p>
                        </div>
                        <div style={{ padding: '16px', background: 'var(--panel-bg)', borderRadius: '12px', border: '1px solid rgba(45, 51, 67, 0.5)' }}>
                            <p style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--muted)', margin: 0 }}>Expired</p>
                            <p className="mono" style={{ fontSize: '20px', margin: '4px 0 0 0', color: 'var(--text)' }}>{admin.number ? admin.number((reservations.expired || []).length) : 0}</p>
                        </div>
                        <div style={{ padding: '16px', background: 'var(--panel-bg)', borderRadius: '12px', border: '1px solid rgba(45, 51, 67, 0.5)' }}>
                            <p style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--muted)', margin: 0 }}>Consumed</p>
                            <p className="mono" style={{ fontSize: '20px', margin: '4px 0 0 0', color: 'var(--text)' }}>{admin.number ? admin.number((reservations.consumed || []).length) : 0}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '24px', alignItems: 'start' }}>
                <div className="admin-card">
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                        <h3 style={{ fontSize: '15px', fontWeight: 800, margin: 0, color: 'var(--text)' }}>System Health</h3>
                    ...
                    <div style={{ display: 'grid', gap: '8px', fontSize: '14px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', itemsCenter: 'center', padding: '12px', background: 'var(--panel-bg)', borderRadius: '6px', border: '1px solid rgba(45, 51, 67, 0.4)' }}>
                            <span style={{ color: 'var(--muted)' }}>Mongo + Redis</span>
                            <span className={`status-badge ${systemHealth.dependencies === 'up' ? 'status-live' : 'status-cancelled'}`}>{systemHealth.dependencies}</span>
                        </div>
                    </div>
                    <p style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '12px' }}>{systemHealth.lastCheckedAt ? `Last checked: ${admin.date ? admin.date(systemHealth.lastCheckedAt) : ''}` : ''}</p>
                </div>
                <div className="admin-card no-pad" style={{ gridColumn: 'span 2' }}>
                    <div style={{ padding: '20px', borderBottom: '1px solid rgba(45, 51, 67, 0.8)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h3 style={{ fontSize: '15px', fontWeight: 800, margin: 0, color: 'var(--text)' }}>Monthly Revenue Report</h3>
                        ...
                        <table className="admin-table">
                            ...
                            <tbody>
                                {(financialSummary.monthlyReport || []).map(row => (
                                    <tr key={row.month}>
                                        <td className="mono" style={{ color: 'var(--text)', fontWeight: 700 }}>{row.month}</td>
                                        <td className="mono">{admin.number ? admin.number(row.orders) : ''}</td>
                                        <td className="mono" style={{ color: 'var(--primary)', fontWeight: 700 }}>{admin.currency ? admin.currency(row.gmv) : ''}</td>
                                ...
                                {!(financialSummary.monthlyReport || []).length && (
                                    <tr>
                                        <td colSpan="5" style={{ padding: '40px', textAlign: 'center', color: 'var(--muted)' }}>No monthly data.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
            ...
            <div className="admin-card no-pad">
                <div style={{ padding: '20px', borderBottom: '1px solid rgba(45, 51, 67, 0.8)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h3 style={{ fontSize: '15px', fontWeight: 800, margin: 0, color: 'var(--text)' }}>Dispute Resolution Center</h3>
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table className="admin-table">
                        ...
                        <tbody>
                            {disputes.map(item => (
                                <tr key={item.id}>
                                    <td className="mono" style={{ color: 'var(--text)', fontWeight: 700 }}>{item.orderId}</td>
                                    <td>{item.reason}</td>
                                    <td>
                                        <select
                            ...
                            {disputes.length === 0 && (
                                <tr>
                                    <td colSpan="5" style={{ padding: '40px', textAlign: 'center', color: 'var(--muted)' }}>No disputes found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
