...
function SyndicateFooterContent({ mobile = false }: { mobile?: boolean }) {
  return (
    <div className={`relative z-10 pointer-events-auto w-full ${mobile ? 'max-w-md mx-auto grid grid-cols-1 gap-3' : 'max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 place-items-stretch'}`}>
      <div className={`bg-black/80 border border-[#333] backdrop-blur-md flex flex-col justify-center text-left group hover:border-[#FF2A00] transition-colors duration-500 ${mobile ? 'rounded-2xl p-5' : 'rounded-[28px] p-6 md:p-12'}`}>
        ...
        <form className={`flex flex-col w-full ${mobile ? 'gap-2.5' : 'gap-3 md:gap-4'}`} onSubmit={(e) => e.preventDefault()}>
          <input
            type="email"
          ...
          />
          <button type="submit" className={`bg-[#FF2A00] hover:bg-[#FFD500] text-black font-display uppercase transition-colors duration-300 w-full flex items-center justify-between ${mobile ? 'text-sm px-4 py-3 rounded-xl tracking-[0.18em]' : 'text-base sm:text-lg px-6 sm:px-8 py-3 sm:py-4 tracking-widest'}`}>
            <span>Transmit</span>
      ...
      <div className={`flex flex-col ${mobile ? 'gap-3' : 'gap-4 md:gap-6'}`}>
        ...
        >
          ...
          <div className="flex flex-col items-start text-left overflow-hidden min-w-0">
            ...
            <span className={`font-display tracking-wider text-white truncate w-full ${mobile ? 'text-lg' : 'text-base sm:text-xl md:text-2xl'}`}>+1 (555) 019-8372</span>
          </div>
        </a>
        ...
        >
          ...
          <div className="flex flex-col items-start text-left overflow-hidden min-w-0">
            ...
            <span className={`font-display tracking-wider text-white truncate w-full ${mobile ? 'text-lg' : 'text-base sm:text-xl md:text-2xl'}`}>Facebook</span>
          </div>
        </a>
      </div>
    </div>
      ...
      <Scroll html style={{ width: '100%' }}>
        ...
        <section className="min-h-[100svh] py-24 pb-40 md:pb-24 flex items-center justify-center text-center px-4 relative pointer-events-none">
          <div className="w-full md:hidden">
            <SyndicateFooterContent mobile />
          </div>
          <div className="hidden w-full md:block">
            <SyndicateFooterContent />
          </div>
        </section>
      </Scroll>
        </>
